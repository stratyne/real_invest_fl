...
import { listProfiles, createProfile, updateProfile, cloneProfile } from '../api/profiles'
import type {
  UserProfile,
  DashboardResponse,
...
function profileToFilterState(p: FilterProfileResponse): FilterState {
  ...
  function range(key: string) {
    return (f[key] ?? {}) as Record<string, unknown>
  }
  function includeList(key: string): unknown[] {
    return ((f[key] as Record<string, unknown>)?.include as unknown[]) ?? []
  }
  function numVal(key: string): number | null {
    return ((f[key] as Record<string, unknown>)?.value as number) ?? null
  }
  function boolReq(key: string): boolean | null {
    const v = (f[key] as Record<string, unknown>)?.required
    return v == null ? null : Boolean(v)
  }
  function excludeList(key: string): string[] {
    return ((f[key] as Record<string, unknown>)?.exclude as string[]) ?? []
  }
  ...
  return {
    ...
    filter_criteria: {
      ...
      filters: {
        ...
        // excluded: disaster_code_present, geometry_filter, min_arv_confidence
      },
    },
...
function NumInput({
  ...
  return (
    <label style={inputStyles.label}>
      <span style={inputStyles.labelText}>{label}</span>
      <input
...
function TextListInput({
  ...
  return (
    <label style={inputStyles.label}>
      ...
      />
      {hint && <span style={inputStyles.hint}>{hint}</span>}
    </label>
  )
}

function NumListInput({
  label, value, onChange, placeholder, hint,
}: {
  label: string
  value: number[]
  ...
  return (
    <div style={modalStyles.overlay}>
      <div style={modalStyles.modal}>
        <h3 style={modalStyles.title}>Save Filter Profile</h3>
        <label style={inputStyles.label}>
          <span style={inputStyles.labelText}>Profile name</span>
          <input
            style={inputStyles.input}
            value={name}
        ...
        <div style={modalStyles.actions}>
          ...
          <button
            style={{ ...modalStyles.saveBtn, opacity: !name.trim() || saving ? 0.6 : 1 }}
...
export default function DashboardPage() {
  ...
  const canSearch = activeCount >= 2 && selectedFips != null

  // ── On mount — load user + dashboard, restore state if returning from results
  useEffect(() => {
    Promise.all([getMe(), getDashboard()])
      .then(([u, d]) => { setUser(u); setDashboard(d) })
      .catch(() => setLoadError('Failed to load dashboard.'))

    const restored = (location.state as { filterState?: FilterState; countyFips?: string } | null)
    if (restored?.filterState && restored?.countyFips) {
      setFilterState(restored.filterState)
      setSelectedFips(restored.countyFips)
    }
  }, [])

  // ── Load profiles when county changes
  useEffect(() => {
    if (!selectedFips) return
    listProfiles(selectedFips)
      .then((ps) => {
        setProfiles(ps)
        setSelectedProfileId(ps.length > 0 ? ps[0].id : null)
        // Don't overwrite filterState if we just restored from results page
        const restored = (location.state as { filterState?: FilterState } | null)
        if (!restored?.filterState) {
          setFilterState(ps.length > 0 ? profileToFilterState(ps[0]) : EMPTY_FILTER)
        }
      })
      .catch(() => setLoadError('Failed to load filter profiles.'))
  }, [selectedFips])

  // ── When profile selection changes, hydrate filter state
  ...
  const handleSave = useCallback(async (name: string) => {
    ...
    setSaveError(null)
    try {
      const payload = filterStateToPayload(filterState, selectedFips, name)
  ...
  if (loadError) return <div style={pageStyles.centerMsg}>{loadError}</div>
  if (!user || !dashboard) return <div style={pageStyles.centerMsg}>Loading…</div>

  const totalActive = dashboard.total_active_listings
    ...
    <div style={pageStyles.outer}>
      ...
      <header style={pageStyles.header}>
        ...
        <div style={pageStyles.headerRight}>
          <span style={pageStyles.userName}>{user.full_name ?? user.email}</span>
          <button style={pageStyles.signOutBtn} onClick={handleSignOut}>Sign out</button>
        </div>
      </header>

      <div style={pageStyles.body}>
        {/* ── Zone 1 — Summary ── */}
        <div style={pageStyles.zone1}>
          <div style={pageStyles.summaryTile}>
            <span style={pageStyles.summaryValue}>{totalActive.toLocaleString()}</span>
            <span style={pageStyles.summaryLabel}>Total Active Listings</span>
          </div>
        </div>

        {/* ── Zone 2 — County tiles ── */}
        <div style={pageStyles.zone2}>
          <h3 style={pageStyles.zoneTitle}>Counties</h3>
          <div style={pageStyles.countyGrid}>
            {dashboard.counties.map((c) => (
              <div
                key={c.county_fips}
                style={{
                  ...pageStyles.countyTile,
                  ...(selectedFips === c.county_fips ? pageStyles.countyTileSelected : {}),
                }}
                onClick={() => setSelectedFips(c.county_fips)}
              >
                <div style={pageStyles.countyName}>{c.county_name}</div>
                <div style={pageStyles.countyFips}>{c.county_fips}</div>
                <div style={pageStyles.countyStatRow}>
                  <span style={pageStyles.countyStatLabel}>Active listings</span>
                  <span style={pageStyles.countyStatValue}>{c.active_listings.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ── Zone 3 — Filter editor ── */}
        {selectedFips && (
          <div style={pageStyles.zone3}>
            <div style={pageStyles.filterHeader}>
              <h3 style={pageStyles.zoneTitle}>Filter Parameters</h3>
              <div style={pageStyles.profilePicker}>
                <span style={pageStyles.profilePickerLabel}>Load profile:</span>
                <select
                  style={pageStyles.profileSelect}
            ...
            <div style={pageStyles.filterGrid}>
              ...
              <Section title="Location" defaultOpen>
                <NumListInput
                  label="DOR County Numbers"
                  value={filterState.county_nos}
                  onChange={(v) => setFs({ county_nos: v })}
                  placeholder="e.g. 27, 67"
                  hint="Escambia = 27 · Santa Rosa = 67"
                />
                <TextListInput
                  label="ZIP Codes"
                  value={filterState.zip_codes}
                  onChange={(v) => setFs({ zip_codes: v as string[] })}
                  placeholder="e.g. 32501, 32502"
                />
                <TextListInput
                  label="Market Area Codes"
                  value={filterState.mkt_ar_codes}
                  onChange={(v) => setFs({ mkt_ar_codes: v as string[] })}
                  placeholder="comma separated"
                />
                <TextListInput
                  label="Neighbourhood Codes"
                  value={filterState.nbrhd_codes}
                  onChange={(v) => setFs({ nbrhd_codes: v as string[] })}
                  placeholder="comma separated"
                />
                <TextListInput
                  label="Census Block Groups"
                  value={filterState.census_block_groups}
                  onChange={(v) => setFs({ census_block_groups: v as string[] })}
                  placeholder="comma separated"
                />
              </Section>
              ...
              <Section title="Property Type" defaultOpen>
                <NumListInput
                  label="DOR Use Codes"
                  value={filterState.dor_use_code}
                  onChange={(v) => setFs({ dor_use_code: v })}
                  placeholder="e.g. 1"
                  hint="1 = Single family residential"
                />
                <NumInput label="Max Buildings" value={filterState.num_buildings_max} onChange={(v) => setFs({ num_buildings_max: v })} />
                <NumInput label="Max Residential Units" value={filterState.num_residential_units_max} onChange={(v) => setFs({ num_residential_units_max: v })} />
              </Section>
              ...
              <Section title="Physical" defaultOpen>
                ...
                <TextListInput
                  label="Exterior Wall Codes"
                  value={filterState.ext_wall_codes}
                  onChange={(v) => setFs({ ext_wall_codes: v as string[] })}
                  placeholder="comma separated"
                />
                <TextListInput
                  label="Foundation Codes"
                  value={filterState.foundation_codes}
                  onChange={(v) => setFs({ foundation_codes: v as string[] })}
                  placeholder="comma separated"
                />
              ...
              <Section title="Ownership">
                ...
                <TextListInput
                  label="Exclude Owner States"
                  value={filterState.owner_state_dom_exclude}
                  onChange={(v) => setFs({ owner_state_dom_exclude: v as string[] })}
                  placeholder="e.g. FL, GA"
                  hint="2-letter state codes to exclude"
                />
              </Section>
              ...
              <Section title="Sale History">
                ...
                <TextListInput
                  label="Prior Sale Qual Codes"
                  value={filterState.prior_sale_qualification}
                  onChange={(v) => setFs({ prior_sale_qualification: v as string[] })}
                  placeholder="e.g. 01, 02"
                  hint="01 = arm's length · 02 = multi-parcel · 03 = foreclosure"
                />
              ...
              <Section title="Listing">
                ...
                <TextListInput
                  label="Listing Types"
                  value={filterState.listing_types}
              ...
              <Section title="Output">
                <NumInput label="Max Results" value={filterState.max_results} onChange={(v) => setFs({ max_results: v })} placeholder="Leave blank for all" />
                <label style={inputStyles.label}>
                  <span style={inputStyles.labelText}>Sort By</span>
                  <select
                    style={inputStyles.select}
                    value={filterState.sort_by_field}
                ...
                <label style={inputStyles.label}>
                  <span style={inputStyles.labelText}>Direction</span>
                  <select
                    style={inputStyles.select}
                    value={filterState.sort_by_direction}
            ...
            {/* ── Zone 4 — Actions ── */}
            <div style={pageStyles.zone4}>
              <div style={pageStyles.filterCount}>
                {activeCount} filter{activeCount !== 1 ? 's' : ''} active
                {activeCount < 2 && (
                  <span style={pageStyles.filterCountWarning}> — minimum 2 required to search</span>
                )}
              </div>
              {saveSuccess && <span style={pageStyles.saveSuccess}>{saveSuccess}</span>}
              <div style={pageStyles.actionBtns}>
                <button
                  style={{ ...pageStyles.saveBtn, opacity: activeCount < 2 ? 0.5 : 1 }}
                ...
                <button
                  style={{ ...pageStyles.searchBtn, opacity: canSearch ? 1 : 0.5 }}
        ...
        {!selectedFips && (
          <div style={pageStyles.noCounty}>
            Select a county above to configure filter parameters.
          </div>
        )}
      </div>
...
const pageStyles: Record<string, React.CSSProperties> = {
  ...
  body: { padding: '24px 32px', maxWidth: '1400px', margin: '0 auto', width: '100%' },
  zone1: { marginBottom: '20px' },
  summaryTile: {
  ...
  summaryLabel: { fontSize: '12px', color: 'var(--color-text-muted)' },
  zone2: { marginBottom: '24px' },
  zoneTitle: { fontSize: '13px', fontWeight: 600, color: 'var(--color-text-muted)', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.06em' },
  countyGrid: { display: 'flex', flexWrap: 'wrap', gap: '12px' },
  countyTile: {
    background: 'var(--color-surface)', border: '1px solid var(--color-border)',
    borderRadius: '10px', padding: '16px 20px', cursor: 'pointer',
    minWidth: '180px', transition: 'border-color 0.15s',
  },
  countyTileSelected: { borderColor: 'var(--color-primary)', background: 'rgba(59,130,246,0.06)' },
  countyName: { fontWeight: 600, fontSize: '15px', marginBottom: '2px' },
  countyFips: { fontSize: '11px', color: 'var(--color-text-muted)', marginBottom: '10px' },
  countyStatRow: { display: 'flex', justifyContent: 'space-between', gap: '16px' },
  countyStatLabel: { fontSize: '11px', color: 'var(--color-text-muted)' },
  countyStatValue: { fontSize: '11px', fontWeight: 600 },
  zone3: { background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '12px', padding: '20px 24px' },
  filterHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' },
  profilePicker: { display: 'flex', alignItems: 'center', gap: '10px' },
  profilePickerLabel: { fontSize: '12px', color: 'var(--color-text-muted)' },
  profileSelect: {
    background: 'var(--color-bg)', border: '1px solid var(--color-border)',
    borderRadius: '6px', color: 'var(--color-text)', padding: '6px 10px', fontSize: '12px',
  },
  filterGrid: {
    display: 'grid',
  ...
  searchBtn: {
    ...
    color: '#fff', padding: '10px 24px', fontWeight: 600, fontSize: '13px',
  },
  noCounty: { padding: '40px', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '14px' },
...
const sectionStyles: Record<string, React.CSSProperties> = {
  ...
  body: { padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '10px', background: 'var(--color-surface)' },
}
